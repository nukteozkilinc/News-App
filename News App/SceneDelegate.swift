...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let notFirstInApp: Bool = UserDefaults.standard.bool(forKey: "notFirstInApp")
        let isLogin: Bool = UserDefaults.standard.bool(forKey: "isLogin")
        
        if notFirstInApp {
            if isLogin {
                guard let homePageViewController = storyboard.instantiateViewController(withIdentifier: "TabBarViewController") as? TabBarViewController else { return }
                window.rootViewController = UINavigationController(rootViewController: homePageViewController)
                
                window.makeKeyAndVisible()
                self.window = window
            } else {
                guard let loginPageViewController =  storyboard.instantiateViewController(withIdentifier: "LoginViewController") as? LoginViewController else {
                    return
                }
